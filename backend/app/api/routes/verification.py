...
async def websocket_verify_endpoint(websocket: WebSocket, phone_number: str):
    ...
    # Accept the socket first so we can send a structured JSON error if needed.
    # Closing before accept() results in a browser-side code 1006 (abnormal closure)
    # with no reason, making it impossible to distinguish "not enrolled" from a crash.
